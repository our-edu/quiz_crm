...
const handlePageHide = () => {
	if (activeQuestion.value > 0 && !quizSubmission.data) {
		...
		navigator.sendBeacon(
			'/api/method/quiz_crm.quiz_crm.doctype.quiz.quiz.submit_quiz?' +
				params.toString()
		)
	}
}
...
const quizSubmission = createResource({
	url: 'quiz_crm.quiz_crm.doctype.quiz.quiz.submit_quiz',
	makeParams(values) {
		return {
			quiz: quiz.data.name,
			results: localStorage.getItem(quiz.data.title),
...
const questionDetails = createResource({
	url: 'quiz_crm.quiz_crm.utils.get_question_details',
	makeParams(values) {
		return {
...
const checkAnswer = () => {
	...
	createResource({
		url: 'quiz_crm.quiz_crm.doctype.quiz.quiz.check_answer',
		params: {
			question: currentQuestion.value,
...
const markLessonProgress = () => {
	...
	if (lessonIndex.length == 2) {
		call('quiz_crm.quiz_crm.api.mark_lesson_progress', {
			course: pathname[3],
