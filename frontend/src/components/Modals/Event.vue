...
const evaluationResource = createResource({
	url: 'quiz_crm.quiz_crm.api.save_evaluation_details',
	makeParams(values) {
		return {
...
const certificateResource = createResource({
	url: 'quiz_crm.quiz_crm.api.save_certificate_details',
	makeParams(values) {
		return {
			member: props.event.member,
			course: props.event.course,
...
const openLink = (type, name) => {
	let url = ''
	if (type === 'course') {
		url = `/quiz/courses/${name}`
	} else if (type === 'batch') {
		url = `/quiz/batches/${name}#students`
	}
