...
onMounted(() => {
	...
	addKeyboardShortcut()
	updateSidebarLinks()
	socket.on('publish_quiz_notifications', (data) => {
...
const getFirstCourse = async () => {
	...
	return await call('quiz_crm.quiz_crm.onboarding.get_first_course')
}

const getFirstBatch = async () => {
	let firstBatch = localStorage.getItem('firstBatch')
	if (firstBatch) return firstBatch
	return await call('quiz_crm.quiz_crm.onboarding.get_first_batch')
}
...
onUnmounted(() => {
	socket.off('publish_quiz_notifications')
})
</script>
