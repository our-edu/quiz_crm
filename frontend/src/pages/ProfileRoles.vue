	...
	<div class="mt-7">
		...
		<div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-5">
			<Switch
				...
				"
				v-model="quiz_student"
				@update:modelValue="saveRole('quiz_student')"
			/>
			<Switch
				size="sm"
...
const roles = createResource({
	url: 'quiz_crm.quiz_crm.utils.get_roles',
	makeParams(values) {
		return {
			name: values.member,
		}
...
const saveRole = async (role) => {
	...
	await call('quiz_crm.quiz_crm.api.save_role', {
