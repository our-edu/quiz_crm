...
from quiz_crm.quiz_crm.utils import get_quiz_path, get_quiz_route
...
def get_context():
	...
	favicon = frappe.db.get_single_value("Website Settings", "favicon") or "/assets/quiz_crm/frontend/favicon.png"
	title = frappe.db.get_single_value("Website Settings", "app_name") or "Quiz CRM"

	context.meta = get_meta(app_path, title, favicon)
	context.title = title
	context.favicon = favicon

	capture("active_site", "quiz_crm")
	return context
...
def get_meta(app_path, title, favicon):
	...
	description = frappe.db.get_single_value("Settings", "meta_description")
	image = frappe.db.get_single_value("Settings", "meta_image")
	keywords = frappe.db.get_single_value("Settings", "meta_keywords")
...
def get_meta_from_document(app_path):
	...
	if re.match(r"^courses/.*$", app_path):
		if "new/edit" in app_path:
			return {
				...
				"link": get_quiz_route("courses/new/edit"),
			}
		...
		return {
			...
			"link": get_quiz_route(f"courses/{course_name}"),
		}
	...
	if re.match(r"^batches/details/.*$", app_path):
		...
		return {
			...
			"link": get_quiz_route(f"batches/details/{batch_name}"),
		}
	...
	if re.match(r"^batches/.*$", app_path):
		...
		if "new/edit" in app_path:
			return {
				...
				"link": get_quiz_route("batches/new/edit"),
			}
		batch = frappe.db.get_value(
			"Batch",
			batch_name,
		...
		return {
			...
			"description": batch.batch_details,
			"keywords": f"{batch.category} {batch.medium}",
			"link": get_quiz_route(f"batches/{batch_name}"),
		}
	...
	if app_path == "job-openings":
		return {
			...
			"link": get_quiz_route("job-openings"),
		}
	...
	if re.match(r"^job-openings/.*$", app_path):
		...
		return {
			...
			"keywords": "Job Openings, Jobs, Vacancies",
			"link": get_quiz_route(f"job-openings/{job_opening_name}"),
		}
	...
	if app_path == "statistics":
		return {
			...
			"link": get_quiz_route("statistics"),
		}
	...
	if re.match(r"^user/.*$", app_path):
		...
		return {
			...
			"link": get_quiz_route(f"user/{username}"),
		}
	...
	if re.match(r"^badges/.*/.*$", app_path):
		...
		return {
			...
			"link": get_quiz_route(f"badges/{badgeName}/{email}"),
		}
	...
	if re.match(r"^quizzes/[^/]+$", app_path):
		...
		if quiz:
			return {
				"title": quiz.title,
				"keywords": quiz.title,
				"link": get_quiz_route(f"quizzes/{quiz_name}"),
			}
	...
	if app_path == "assignments":
		return {
			...
			"link": get_quiz_route("assignments"),
		}
	...
	if re.match(r"^assignments/[^/]+$", app_path):
		...
		if assignment:
			return {
				...
				"link": get_quiz_route(f"assignments/{assignment_name}"),
			}
	...
	if app_path == "certified-participants":
		return {
			...
			"link": get_quiz_route("certified-participants"),
		}
