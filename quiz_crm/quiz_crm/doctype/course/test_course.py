...
from quiz_crm.quiz_crm.api import delete_course
from quiz_crm.quiz_crm.test_helpers import BaseTestUtils


class TestCourse(BaseTestUtils):
	def setUp(self):
		super().setUp()
	...
	def test_delete_course(self):
		...
		self._create_user(user_email, "Test", "Member", ["Quiz Student"])
		...
		self.assertFalse(frappe.db.exists("Course", course.name))
		...
		self.assertFalse(frappe.db.exists("Course Progress", {"course": course.name}))
		...
		self.cleanup_items.remove(("Course", course.name))
		self.cleanup_items.remove(("Enrollment", enrollment.name))
		self.cleanup_items.remove(("Course Progress", progress.name))
