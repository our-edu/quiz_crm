...
class Course(Document):
	...
	def on_payment_authorized(self, payment_status):
		if payment_status in ["Authorized", "Completed"]:
			update_payment_record("Course", self.name)

	def send_email_to_interested_users(self):
		interested_users = frappe.get_all("Course Interest", {"course": self.name}, ["name", "user"])
		subject = self.title + " is available!"
		args = {
			"title": self.title,
			"course_link": get_quiz_route(f"courses/{self.name}"),
		...
		for user in interested_users:
			...
			email_args = frappe._dict(
				...
				template="course_interest",
				args=args,
				now=True,
			)
			frappe.enqueue(method=frappe.sendmail, queue="short", timeout=300, is_async=True, **email_args)
			frappe.db.set_value("Course Interest", user.name, "email_sent", True)

	def autoname(self):
		if not self.name:
			self.name = generate_slug(self.title, "Course")
...
def send_notification_for_published_courses():
	send_notification = frappe.db.get_single_value("Settings", "send_notification_for_published_courses")
...
def send_email_notification_for_published_courses(courses):
	...
	for course in courses:
		instructors = get_instructors("Course", course.name)
		...
		args = {
			...
			"course_url": frappe.utils.get_url(get_quiz_route(f"courses/{course.name}")),
		}
		...
		frappe.db.set_value("Course", course.name, "notification_sent", 1)
		...
		notification = frappe._dict(
			{
				...
				"type": "Alert",
				"link": get_quiz_route(f"courses/{course.name}"),
			}
		)
...
def update_course_statistics():
	...
	for course in courses:
		...
		enrollments = frappe.db.count("Enrollment", {"course": course.name, "member_type": "Student"})
