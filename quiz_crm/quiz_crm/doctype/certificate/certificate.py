...
class Certificate(Document):
	...
	def send_mail(self):
		...
		custom_template = frappe.db.get_single_value("Settings", "certification_template")

		args = {
			"student_name": self.member_name,
			"course_name": self.course,
			"course_title": frappe.db.get_value("Course", self.course, "title"),
	...
	def validate_batch_enrollment(self):
		if self.batch_name:
			is_enrolled = frappe.db.exists(
				"Batch Enrollment", {"batch": self.batch_name, "member": self.member}
			)
			if not is_enrolled:
	...
	def validate_course_enrollment(self):
		if self.course:
			is_enrolled = frappe.db.exists("Enrollment", {"course": self.course, "member": self.member})
			if not is_enrolled:
				frappe.throw(
					_("Certification cannot be issued as the member is not enrolled in this course.")
				)

			completion_certificate = frappe.db.get_value("Course", self.course, "enable_certification")
			if completion_certificate:
				progress = frappe.db.get_value(
					"Enrollment", {"course": self.course, "member": self.member}, "progress"
				)
				if progress < 100:
					frappe.throw(
...
def is_certified(course):
	certificate = frappe.get_all("Certificate", {"member": frappe.session.user, "course": course})
	if len(certificate):
...
def create_certificate(course: str):
	...
	if certificate:
		return frappe.db.get_value(
			"Certificate", certificate, ["name", "course", "template"], as_dict=True
		)
...
def validate_certification_eligibility(course):
	if not frappe.db.exists("Enrollment", {"course": course, "member": frappe.session.user}):
		frappe.throw(_("You are not enrolled in this course."))

	if not frappe.db.get_value("Course", course, "enable_certification"):
		frappe.throw(_("Certification is not enabled for this course."))

	progress = frappe.db.get_value(
		"Enrollment", {"course": course, "member": frappe.session.user}, "progress"
	)
	if progress < 100:
