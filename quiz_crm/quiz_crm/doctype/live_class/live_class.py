...
from quiz_crm.quiz_crm.doctype.batch.batch import authenticate
...
class LiveClass(Document):
	...
	def get_participants(self):
		participants = frappe.get_all("Batch Enrollment", {"batch": self.batch_name}, pluck="member")
		instructors = frappe.get_all(
			"Course Instructor", {"parenttype": "Batch", "parent": self.batch_name}, pluck="instructor"
		)
	...
	def create_calendar_event(self):
		if self.conferencing_provider == "Google Meet":
			calendar = frappe.db.get_value(
				"Google Meet Settings", self.google_meet_account, "google_calendar"
			)
		else:
			calendar = frappe.db.get_value(
...
def create_attendance(live_class, data):
	for participant in data:
		doc = frappe.new_doc("Live Class Participant")
...
def update_attendees_count(live_class, data):
	frappe.db.set_value("Live Class", live_class.name, "attendees", len(data))
