...
class TestLMSLiveClass(BaseTestUtils):
	...
	def _setup_google_meet(self):
		...
		self.cleanup_items.append(("Google Meet Settings", self.google_meet_settings.name))
	...
	def _create_live_class(self, provider="Google Meet", **kwargs):
		...
		self.cleanup_items.append(("Live Class", live_class.name))
		return live_class
	...
	def test_google_meet_disabled_account_raises_error(self):
		...
		from quiz_crm.quiz_crm.doctype.batch.batch import create_google_meet_live_class
	...
	def test_delete_live_class_deletes_event(self):
		...
		self.cleanup_items = [
			(t, n) for t, n in self.cleanup_items if not (t == "Live Class" and n == live_class.name)
		]
		frappe.delete_doc("Live Class", live_class.name, force=True)
