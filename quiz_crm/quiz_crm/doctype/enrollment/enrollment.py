...
class Enrollment(Document):
	...
	def validate_course_enrollment_eligibility(self):
		...
		if self.enrollment_from_batch:
			...
			if frappe.db.exists(
				"Batch Enrollment", {"batch": self.enrollment_from_batch, "member": self.member}
			):
				return
		...
		if course_details.paid_course:
			payment = frappe.db.exists(
				"Payment",
				{
					"payment_for_document_type": "Course",
...
def update_program_progress(member):
	programs = frappe.get_all("Program Member", {"member": member}, ["parent", "name"])

	for program in programs:
		total_progress = 0
		courses = frappe.get_all("Program Course", {"parent": program.parent}, pluck="course")
		for course in courses:
			progress = frappe.db.get_value("Enrollment", {"course": course, "member": member}, "progress")
			progress = progress or 0
			total_progress += progress

		average_progress = ceil(total_progress / len(courses))
		frappe.db.set_value("Program Member", program.name, "progress", average_progress)
