...
from quiz_crm.quiz_crm.doctype.badge.badge import eval_condition


class BadgeAssignment(Document):
	def validate(self):
	...
	def validate_owner(self):
		event = frappe.db.get_value("Badge", self.badge, "event")
	...
	def validate_duplicate_badge_assignment(self):
		grant_only_once = frappe.db.get_value("Badge", self.badge, "grant_only_once")
	...
	def validate_badge_criteria(self):
		badge_details = frappe.db.get_value(
			"Badge", self.badge, ["reference_doctype", "user_field", "condition", "enabled"], as_dict=True
		)
