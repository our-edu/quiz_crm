...
from quiz_crm.quiz_crm.utils import get_evaluator
...
class CertificateRequest(Document):
	...
	def validate_if_existing_requests(self):
		...
		for req in existing_requests:
			if (
				...
				course_title = frappe.db.get_value("Course", req.course, "title")
				frappe.throw(
	...
	def validate_evaluation_end_date(self):
		if self.batch_name:
			evaluation_end_date = frappe.db.get_value("Batch", self.batch_name, "evaluation_end_date")
...
def schedule_evals():
	if frappe.db.get_single_value("Settings", "send_calendar_invite_for_evaluations"):
...
def update_meeting_details(evaluation: dict, event: Document, calendar: str):
	...
	event.reload()
	frappe.db.set_value(
		"Certificate Request", evaluation.name, "google_meet_link", event.google_meet_link
	)
...
def create_lms_certificate_evaluation(source_name: str, target_doc: dict = None):
	...
	doc = get_mapped_doc(
		"Certificate Request",
		source_name,
		{"Certificate Request": {"doctype": "Certificate Evaluation"}},
		target_doc,
	)
	return doc
...
def mark_eval_as_completed():
	...
	for req in requests:
		if req.date < getdate():
			frappe.db.set_value("Certificate Request", req.name, "status", "Completed")
