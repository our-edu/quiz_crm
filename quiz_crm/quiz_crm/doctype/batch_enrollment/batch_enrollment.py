...
class BatchEnrollment(Document):
	...
	def validate_payment(self):
		paid_batch = frappe.db.get_value("Batch", self.batch, "paid_batch")
		if paid_batch:
			payment = frappe.db.exists(
				"Payment",
				{
					"payment_for_document_type": "Batch",
	...
	def validate_self_enrollment(self):
		batch_details = frappe.db.get_value(
			"Batch", self.batch, ["allow_self_enrollment", "paid_batch"], as_dict=True
		)
	...
	def validate_seat_availability(self):
		seat_count = frappe.db.get_value("Batch", self.batch, "seat_count")
		enrolled_count = frappe.db.count("Batch Enrollment", {"batch": self.batch})
	...
	def add_member_to_live_class(self):
		live_classes = frappe.get_all("Live Class", {"batch_name": self.batch}, ["name", "event"])
...
def send_mail(doc):
	...
	custom_template = batch.confirmation_email_template or frappe.db.get_single_value(
		"Settings", "batch_confirmation_template"
	)
