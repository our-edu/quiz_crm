...
from quiz_crm.quiz_crm.utils import get_quiz_route
...
def allowed_payment_types():
	send_batch_reminders = frappe.db.get_single_value("Settings", "send_payment_reminders_for_batch")
	send_course_reminders = frappe.db.get_single_value("Settings", "send_payment_reminders_for_course")

	allowed_types = []
	if send_batch_reminders:
		allowed_types.append("Batch")

	if send_course_reminders:
		allowed_types.append("Course")
...
def is_batch_sold_out(payment):
	if payment.payment_for_document_type == "Batch":
		seat_count = frappe.get_cached_value("Batch", payment.payment_for_document, "seat_count")
		number_of_students = frappe.db.count("Batch Enrollment", {"batch": payment.payment_for_document})
...
def send_mail(payment):
	...
	custom_template = frappe.db.get_single_value("Settings", "payment_reminder_template")
