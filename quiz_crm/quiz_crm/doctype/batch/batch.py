...
class Batch(Document):
	...
	def autoname(self):
		if not self.name:
			self.name = generate_slug(self.title, "Batch")
	...
	def validate_duplicate_courses(self):
		...
		if len(duplicates):
			title = frappe.db.get_value("Course", next(iter(duplicates)), "title")
	...
	def validate_seats_left(self):
		...
		students = frappe.db.count("Batch Enrollment", {"batch": self.name})
	...
	def validate_conferencing_provider(self):
		...
		if self.conferencing_provider == "Google Meet":
			...
			google_meet_settings = frappe.get_doc("Google Meet Settings", self.google_meet_account)
	...
	def on_payment_authorized(self, payment_status):
		if payment_status in ["Authorized", "Completed"]:
			update_payment_record("Batch", self.name)


def send_notification_for_published_batch(batch):
	send_notification = frappe.db.get_single_value("Settings", "send_notification_for_published_batches")
...
def send_email_notification_for_published_batch(batch):
	...
	instructors = get_instructors("Batch", batch.name)
	...
	args = {
		...
		"batch_url": frappe.utils.get_url(get_quiz_route(f"batches/{batch.name}")),
	}
	...
	frappe.db.set_value("Batch", batch.name, "notification_sent", 1)
	...
	notification = frappe._dict(
		{
			...
			"type": "Alert",
			"link": get_quiz_route(f"batches/{batch.name}"),
		}
	)
...
def create_google_meet_live_class(
	...
	google_meet_settings = frappe.get_doc("Google Meet Settings", google_meet_account)
	if not google_meet_settings.enabled:
...
def authenticate(zoom_account):
	zoom = frappe.get_doc("Zoom Settings", zoom_account)
	if not zoom.enabled:
...
def get_batch_timetable(batch: str):
	...
	show_live_class = frappe.db.get_value("Batch", batch, "show_live_class")
	if show_live_class:
...
def get_live_classes(batch):
	...
	for class_ in live_classes:
		...
		class_.reference_doctype = "Live Class"
...
def get_timetable_details(timetable):
	for entry in timetable:
		...
		elif entry.reference_doctype == "Quiz":
			entry.url = "/quizzes"
			details = get_quiz_details(assessment, frappe.session.user)
			entry.update(details)

		elif entry.reference_doctype == "Assignment":
...
def send_batch_start_reminder():
	...
	for batch in batches:
		students = frappe.get_all("Batch Enrollment", {"batch": batch.name}, ["member", "member_name"])
...
def has_permission(doc, ptype="read", user=None):
	...
	is_enrolled = frappe.db.exists("Batch Enrollment", {"batch": doc.name, "member": user})
	if is_enrolled:
		return True

	is_batch_published = frappe.db.get_value("Batch", doc.name, "published")
