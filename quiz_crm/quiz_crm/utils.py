...
from quiz_crm.quiz_crm.doctype.enrollment.enrollment import update_program_progress
from quiz_crm.quiz_crm.md import find_macros

RE_SLUG_NOTALLOWED = re.compile("[^a-z0-9]+")
QUIZ_ROLES = ["Moderator", "Course Creator", "Batch Evaluator", "Quiz Student"]


def get_quiz_path():
	path = frappe.conf.get("quiz_path") or "lms"
	return path.strip("/")
...
def extend_bootinfo(bootinfo: dict):
	bootinfo["quiz_path"] = get_quiz_path()
...
def get_reviews(course: str):
	...
	out_of_ratings = frappe.db.get_all(
		"DocField", {"parent": "Course Review", "fieldtype": "Rating"}, ["options"]
	)
...
def get_lesson_url(course: str, lesson_number: str):
	if not lesson_number:
		return
	return get_quiz_route(f"courses/{course}/learn/{lesson_number}")
...
def is_instructor(course: str) -> bool:
	instructors = get_instructors("Course", course)
	for instructor in instructors:
...
def has_student_role(member: str = None):
	return frappe.db.get_value(
		"Has Role",
		{"parent": member or frappe.session.user, "role": "Quiz Student"},
		"name",
	)
...
def handle_notifications(doc: Document, method: str):
	...
	if topic.reference_doctype not in ["Course Lesson", "Batch"]:
		return
...
def get_course_details_for_notification(topic: dict):
	...
	course_title = frappe.db.get_value("Course", course, "title")
	instructors = frappe.db.get_all(
		"Course Instructor", {"parent": course, "parenttype": "Course"}, pluck="instructor"
	)
...
def get_batch_details_for_notification(topic: dict):
	users = []
	batch_title = frappe.db.get_value("Batch", topic.reference_docname, "title")
	subject = _("New comment in batch {0}").format(batch_title)
	link = get_quiz_route(f"batches/{topic.reference_docname}#discussions")
	instructors = frappe.db.get_all(
		"Course Instructor",
		{"parenttype": "Batch", "parent": topic.reference_docname},
		pluck="instructor",
	)
	students = frappe.db.get_all("Batch Enrollment", {"batch": topic.reference_docname}, pluck="member")
	users += instructors
	users += students
	return subject, link, users
...
def notify_mentions_via_email(doc: Document, topic: dict):
	...
	if topic.reference_doctype == "Batch":
...
def get_course_completion_data():
	all_membership = frappe.db.count("Enrollment")
	completed = frappe.db.count("Enrollment", {"progress": ["like", "%100%"]})
...
def apply_gst(amount: float, country: str = None) -> tuple:
	gst_applied = 0
	apply_gst = frappe.db.get_single_value("Settings", "apply_gst")
...
def guest_access_allowed():
	allow_guest_access = frappe.get_cached_value("Settings", None, "allow_guest_access")
...
def get_course_card_details(courses: list) -> list:
	for course in courses:
		course.instructors = get_instructors("Course", course.name)
...
def update_course_filters(filters: dict) -> tuple:
	...
	if filters.get("enrolled"):
		enrolled_courses = frappe.get_all("Enrollment", {"member": frappe.session.user}, pluck="course")
...
def get_course_details(course: str):
	...
	is_course_published = frappe.db.get_value("Course", course, "published")
	...
	course_details.instructors = get_instructors("Course", course_details.name)
...
def get_lesson(course: str, chapter: int, lesson: int) -> dict:
	...
	lesson_details.icon = get_lesson_icon(lesson_details.body, lesson_details.content)
	lesson_details.instructors = get_instructors("Course", course)
...
def get_batch_details(batch: str):
	...
	batch_students = frappe.get_all("Batch Enrollment", {"batch": batch}, pluck="member")
	is_batch_admin = can_modify_batch(batch)
	is_batch_published = frappe.db.get_value("Batch", batch, "published")
	...
	batch_details.instructors = get_instructors("Batch", batch)
	...
	batch_details.assessments = frappe.get_all(
		"Assessment", {"parent": batch}, ["assessment_name", "assessment_type"]
	)
...
def categorize_batches(batches: list) -> dict:
	...
	for batch in batches:
		...
		if frappe.session.user != "Guest":
			if frappe.db.exists("Batch Enrollment", {"member": frappe.session.user, "batch": batch.name}):
...
def get_assessments(batch: str) -> list:
	member = frappe.session.user
	is_enrolled = frappe.db.exists("Batch Enrollment", {"batch": batch, "member": member})
	...
	for assessment in assessments:
		...
		elif assessment.assessment_type == "Programming Exercise":
...
def get_assignment_details(assessment: dict, member: str) -> dict:
	assessment.title = frappe.db.get_value("Assignment", assessment.assessment_name, "title")
	...
	assessment.url = get_quiz_route(f"assignment-submission/{assessment.assessment_name}/{submission_name}")
...
def get_quiz_details(assessment: dict, member: str) -> dict:
	assessment_details = frappe.db.get_value(
		"Quiz", assessment.assessment_name, ["title", "passing_percentage"], as_dict=1
	)
...
def get_exercise_details(assessment: dict, member: str) -> dict:
	assessment.title = frappe.db.get_value("Programming Exercise", assessment.assessment_name, "title")
	filters = {"member": member, "exercise": assessment.assessment_name}

	if frappe.db.exists("Programming Exercise Submission", filters):
		assessment.submission = frappe.db.get_value(
...
def get_course_completion_stats(batch: str) -> list:
	...
	BatchEnrollment = frappe.qb.DocType("Batch Enrollment")
...
def get_assignment_pass_stats(batch: str) -> list:
	...
	Assignment = frappe.qb.DocType("Assignment")
	BatchEnrollment = frappe.qb.DocType("Batch Enrollment")
	Submission = frappe.qb.DocType("Assignment Submission")
	...
	rows = (
		...
		.where((Assessment.parent == batch) & (Assessment.assessment_type == "Assignment"))
...
def get_quiz_pass_stats(batch: str) -> list:
	...
	Assessment = frappe.qb.DocType("Assessment")
	Quiz = frappe.qb.DocType("Quiz")
	BatchEnrollment = frappe.qb.DocType("Batch Enrollment")
	Submission = frappe.qb.DocType("Quiz Submission")
	...
	rows = (
		...
		.where((Assessment.parent == batch) & (Assessment.assessment_type == "Quiz"))
...
def calculate_course_progress(batch_courses: list, details: dict):
	...
	for course in batch_courses:
		progress = (
			frappe.db.get_value(
				"Enrollment", {"course": course.course, "member": details.email}, "progress"
			)
			or 0
		)
...
def get_assessment_meta(assessment_type: str):
	if assessment_type == "Assignment":
		...
		fields = ["percentage"]
		not_attempted = 0
	elif assessment_type == "Programming Exercise":
...
def get_assessment_attempt_details(
	...
	if assessment_type == "Quiz":
		result = "Failed"
		passing_percentage = frappe.db.get_value("Quiz", assessment, "passing_percentage")
	...
	return frappe._dict(
		{
			"status": attempt_details.percentage if assessment_type == "Quiz" else attempt_details.status,
			"result": result,
...
def can_access_topic(doctype: str, docname: str) -> bool:
	...
	if doctype == "Course Lesson":
		...
		is_student = frappe.db.exists("Enrollment", {"course": course, "member": frappe.session.user})
		if not is_student and not can_modify_course(course):
			return False
	elif doctype == "Batch":
		is_student = frappe.db.exists(
			"Batch Enrollment", {"batch": docname, "member": frappe.session.user}
		)
...
def get_order_summary(doctype: str, docname: str, coupon: str = None, country: str = None):
	details = get_paid_course_details(docname) if doctype == "Course" else get_paid_batch_details(docname)
...
def apply_coupon(doctype: str, docname: str, code: str, base_amount: float):
	coupon_name = frappe.db.exists("Coupon", {"code": code, "enabled": 1})
	if not coupon_name:
...
def validate_coupon_applicability(doctype: str, docname: str, coupon_name: str):
	applicable_item = frappe.db.exists(
		"Coupon Item", {"parent": coupon_name, "reference_doctype": doctype, "reference_name": docname}
	)
	if not applicable_item:
		frappe.throw(
			_("This coupon is not applicable to this {0}.").format(
				"Course" if doctype == "Course" else "Batch"
			)
		)
...
def get_roles(name: str) -> dict:
	...
	return {
		...
		"quiz_student": has_student_role(name),
	}


def publish_notifications(doc: Document, method: str):
	frappe.publish_realtime("publish_quiz_notifications", user=doc.for_user, after_commit=True)
...
def get_payment_doc(payment_name: str) -> dict:
	return frappe.db.get_value(
		"Payment", payment_name, ["name", "coupon", "payment_for_certificate"], as_dict=True
	)
...
def update_coupon_redemption(payment_doc: dict):
	if payment_doc.coupon:
		redemption_count = frappe.db.get_value("Coupon", payment_doc.coupon, "redemption_count") or 0
...
def enroll_in_course(course: str, payment_name: str):
	if not frappe.db.exists("Enrollment", {"member": frappe.session.user, "course": course}):
		enrollment = frappe.new_doc("Enrollment")
		payment = frappe.db.get_value("Payment", payment_name, ["name", "source"], as_dict=True)
...
def get_payment_details(payment_name: str) -> dict:
	...
	if payment_name:
		payment_doc = frappe.db.get_value(
			"Payment", payment_name, ["name", "source", "payment_received"], as_dict=True
		)
	return payment_doc


def create_enrollment(batch: str, payment_doc: dict = None):
	new_student = frappe.new_doc("Batch Enrollment")
	new_student.update(
		{
			"member": frappe.session.user,
			"batch": batch,
...
def get_programs():
	...
	enrolled_programs = frappe.get_all(
		"Program Member", {"member": frappe.session.user}, ["parent as name", "progress"]
	)
	for program in enrolled_programs:
		program.update(
			frappe.db.get_value(
				"Program", program.name, ["name", "course_count", "member_count"], as_dict=True
			)
		)
...
def get_program_details(program_name: str) -> dict:
	...
	is_published = frappe.db.get_value("Program", program_name, "published")
	is_member = frappe.db.exists(
		"Program Member", {"parent": program_name, "member": frappe.session.user}
	)
	...
	program_courses = frappe.get_all(
		"Program Course", {"parent": program_name}, ["course"], order_by="idx"
	)
...
def enroll_in_program(program: str):
	...
	if not frappe.db.exists("Program Member", {"parent": program, "member": frappe.session.user}):
		program_member = frappe.new_doc("Program Member")
...
def validate_program_enrollment(program: str):
	published = frappe.db.get_value("Program", program, "published")
	if not published:
...
def get_batches(filters: dict = None, start: int = 0, order_by: str = "start_date"):
	...
	if filters.get("enrolled"):
		enrolled_batches = frappe.get_all(
			"Batch Enrollment", {"member": frappe.session.user}, pluck="batch"
		)
...
def get_batch_card_details(batches: list) -> list:
	for batch in batches:
		batch.instructors = get_instructors("Batch", batch.name)
		students_count = frappe.db.count("Batch Enrollment", {"batch": batch.name})
...
def persona_captured():
	frappe.db.set_single_value("Settings", "persona_captured", 1)
...
def validate_discussion_reply(doc: Document, method: str):
	...
	elif topic.reference_doctype == "Batch":
...
def validate_course_access(lesson: str):
	...
	enrollment_exists = frappe.db.exists("Enrollment", {"member": frappe.session.user, "course": course})
...
def validate_batch_access(batch: str):
	...
	enrollment_exists = frappe.db.exists(
		"Batch Enrollment", {"member": frappe.session.user, "batch": batch}
	)
	if not enrollment_exists:
...
def can_modify_course(course: str) -> bool:
	is_instructor = frappe.db.exists(
		"Course Instructor",
		{"instructor": frappe.session.user, "parent": course, "parenttype": "Course"},
	)
...
def has_quiz_role():
	...
	quiz_roles = set(QUIZ_ROLES)
	user_roles = set(roles)
	return not quiz_roles.isdisjoint(user_roles)
