...
from quiz_crm.quiz_crm.utils import create_user as create_quiz_user
...
def get_exercise_test_cases(doc):
	...
	for tc in doc.test_cases:
		test_case_doc = frappe.get_doc("Test Case", tc.name)
...
def get_assessments_from_lesson(lesson):
	...
	for block in content.get("blocks", []):
		...
		elif doc.doctype == "Programming Exercise":
...
def verify_category(category_name):
	if category_name and not frappe.db.exists("Category", category_name):
		category = frappe.new_doc("Category")
...
def get_assessment_map():
	return {"quiz": "Quiz", "assignment": "Assignment", "program": "Programming Exercise"}
	...
	for block in content.get("blocks", []):
		if block.get("type") == "upload":
			...
			if asset_url:
				...
				current_asset_url = frappe.db.get_value("Asset", {"file_name": asset_name}, "file_url")
...
def add_questions_to_quiz(quiz_doc, questions):
	for question in questions:
		...
		question_name = frappe.db.get_value("Question", {"question": question_detail}, "name")
		if question_name:
...
def build_assessment_doc(assessment_data):
	...
	if doctype not in ("Quiz", "Assignment", "Programming Exercise"):
		return
	...
	elif doctype == "Programming Exercise":
